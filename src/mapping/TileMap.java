...
public class TileMap {
    ...
    public void setMap(int[][] newMap){
        int rows = newMap.length;
        int cols = newMap[0].length;

        setNewMap(cols,rows); //create empty map
        for (int row = 0; row < rows; row++){
            for (int col = 0; col < cols; col++){
                setTileID(row,col,newMap[row][col]);
            }
        }
    }
    public void setMapList(TileMap map){ this.map = map.getMap(); }
